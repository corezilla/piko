...
| F-5 | Piko↔LLMTier 无端到端 request_id 关联、无逐跳时延统计；`logs` 仅 HTTP 访问日志（无上游细节） | 失败定位需依赖 Pi 会话 JSONL + 时间窗关联（已工具化：`scripts/joint-diagnose.sh`） | **大部分已解决（Piko）**：`PIKO_PROVIDER_DEBUG` 环回 + `provider_calls` 持久化（x-request-id==账本 request_id、**hop latency_ms** 实测 3364ms 一致）；剩余上游明细/逐阶段时间戳 → LT-OBS-1/6 |

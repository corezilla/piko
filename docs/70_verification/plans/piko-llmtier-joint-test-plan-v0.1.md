...
> **从开始到结束**的全过程：启动检查 → 17 个 case 按序逐步执行 → 收敛 → 联合多维度测试 → 输出成果归档。
...
- **目标**：完成 Piko ↔ 真实 LLMTier 的首次联合调试（规格 §3 全部 17 个 case，按 JT 编号递增
...
```text
S0 启动检查 ──▶ S1 逐步执行 JT-01 → JT-17（JT-17 待 R-T-5） ──▶ S2 缺陷修复与收敛 ──▶ S3 联合多维度测试 ──▶ S4 报告与归档
...
执行顺序固定：**JT-01 → JT-02 → … → JT-17**（0.1.x 完成 JT-01..13；0.2.0 review 后增补 JT-14/15/16，均已执行；JT-17 白盒注入 case 待 LLMTier 实现 R-T-5/LT-OBS-5 后执行）。
...
| 成果 | 落点 |
|---|---|
| 规格 §3 全部 17 case 状态为终态判定（JT-17 于 R-T-5 实现后补执行） | 规格文档 |
...
normal：JT-01/02/03/04/09/10/11/12/13/15/16；negative：JT-05/06/07/14/17；recovery/fault-injection：JT-08（及
...
- **Exit（联调 Gate）**：17 个 case 全部执行完毕且有终态判定（JT-17 于 R-T-5 实现后补执行并复核） ∧ S3 联合多维度测试完成
...
- **可观测性需求状态**：R-P-1/2/3（Piko）**已实现并实测**（provider 环回开关 + `provider_calls`
  ...
  流注入开关（白盒流程开关），已提需求；实现后执行 JT-17 并由本方 review。
  **R-T-6（LT-OBS-6）**：单请求全生命周期 trace（逐阶段时间戳/导出 JSON/保留 ≥7 天）——供三方定位与报告证据。
  **R-T-7（LT-OBS-7）**：consumer 关联标识透传（logs/usage/trace 回显）——供双向定位。
